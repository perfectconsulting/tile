 ...
 */
void open_strm( strm_name )
  char *strm_name ;
{
  FILE *stream ;
  char pad[128];
  struct file_elmt *new_file_ptr ;
  stream = fopen( dp_string(pad) , "r" ) ;
  if( stream == TILENULL )
    {
...
#endif
  if( fgets( string , MAXLINE , file_ptr->stream ) == TILENULL )
  {
    if( ! terminal() )
	  {
	    printf( "[End of file]\n" ) ;
